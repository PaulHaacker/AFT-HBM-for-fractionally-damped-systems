%% MSM_standardDuffingForced.m
% Multiple-scales (MSM) analysis of the standard forced Duffing oscillator.
%
% system equations:
...
[eta_vec, a_vec] = arclength_continuation(...
    @(x,eta) msm_equilibrium(x, eta - offset, beta, f, omega0, mu_nl), ...
...
is_stable_msm = false(1, num_pts);
for kk = 1:num_pts
    a   = a_vec(kk);
    sig = sigma_vec(kk);
    J   = msm_jacobian(a, sig, beta, f, omega0, mu_nl);
    is_stable_msm(kk) = all(real(eig(J)) < 0);
end
...
%% Combined plot: HBM vs. MSM, one window
% line style encodes stability (solid = stable, dashed = unstable),
% color encodes the method (HBM vs. MSM)
color_hbm = [0, 0.4470, 0.7410];    % MATLAB default blue
color_msm = [0.8500, 0.3250, 0.0980]; % MATLAB default orange

figure; hold on
plot_stability_curve(om, A_HBM, is_stable_hbm, color_hbm)
plot_stability_curve(Om_mms, a_vec, is_stable_msm, color_msm)
...
h_msm = plot(nan, nan, '-', 'Color', color_msm, 'LineWidth', 1.5, 'DisplayName', 'MSM');
legend([h_hbm, h_msm], 'Location', 'best', 'Interpreter', 'latex')
hold off
...
%% -------------------------------------------------------------------
function [F, dFdx, dFdmu] = msm_equilibrium(x, mu, beta, f, omega0, mu_nl)
% Residual and Jacobians of the MSM equilibrium equation for use with
...
%% -------------------------------------------------------------------
function J = msm_jacobian(a, sig, beta, f, omega0, mu_nl)
